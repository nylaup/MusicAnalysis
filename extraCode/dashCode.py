...
import re, dash, base64, calendar, webbrowser, plotly.colors
...
def get_artist_color_map(artists):
    palette = plotly.colors.qualitative.Dark24 
    colors = palette * ((len(artists) // len(palette)) + 1)
    return {artist: color for artist, color in zip(artists, colors)}

def make_topsongs(dataframe, color_map):
...
def make_topartists(dataframe, color_map):
    #For pie chart
    ...
    pie = px.pie(artist10, values='count', names='artist', title="Top 10 Artists", color_discrete_map=color_map)
    line = px.line(monthly_counts, x="month", y="listen_count", color="artist", 
                   title="Top 5 Artists Through the Year", color_discrete_map=color_map)
    line.update_layout(xaxis_title='Month of Year', yaxis_title='Listen Count')
    bar = px.histogram(top5, x='hour', color='artist', nbins=24, barmode='stack',
                        title='Top 5 Artists Through the Day', color_discrete_map=color_map)
...
def monthly_analysis(dataframe, months, color_map):
    ...
    pie = px.pie(top5, values='count', names='artist',
                  title="Top 5 Artists for Select Months", color_discrete_map=color_map)
...
def update_graphs(data, selected_year, selected_months, selected_platforms):
    ...
    all_artists = music['artist'].unique()
    color_map = get_artist_color_map(all_artists)

    top_songs_fig = make_topsongs(music, color_map)
    top_artists_figs = make_topartists(music, color_map)
    monthly_fig = monthly_analysis(music, selected_months, color_map)
